...
class TestPersonalIncomeController(
  val authConnector: AuthConnector, val service: PersonalIncomeService, val confLevel: Int, val logger: LoggerLike,
...
class TestPersonalIncomeRenewalClaimantDetailsSpec extends TestSetup with WithFakeApplication with ClaimsJson {
  ...
  "full claimant details" should {
    ...
    "return details with the renewalFormType set" in new mocks {
      ...
      stubClaimantDetailsResponse(ClaimantDetails(false, 1, renewalFormType, incorrectNino.value, None, false, "some-app-id"))

      val result = await(controller.fullClaimantDetails(Nino(nino), journeyId)(fakeRequest))
      status(result) shouldBe 200
...
class TestPersonalIncomeRenewalSpec extends TestSetup with WithFakeApplication {
  ...
  val submitRenewalRequest = FakeRequest().withBody(toJson(renewal)).withHeaders(
    acceptHeader,
    HeaderKeys.tcrAuthToken -> "some-auth-token"
  )

  val requestIncorrectNoHeader = FakeRequest().withBody(toJson(renewal)).withHeaders(
    HeaderKeys.tcrAuthToken -> "some-auth-token"
  )
  ...
  "renewal Live" should {
    ...
    "return 403 result when no tcr auth header has been supplied" in new mocks {
      stubAuthorisationGrantAccess(Some(nino) and L200)
      val invalidRequest = FakeRequest().withBody(toJson(renewal)).withHeaders(acceptHeader)
      val controller = new LivePersonalIncomeController(mockAuthConnector, 200, logger)
